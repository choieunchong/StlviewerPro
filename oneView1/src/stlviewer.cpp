...
void STLViewer::on_OpentoolButton_clicked()
{
    ...
    mOriginProp = mActor->GetProperty();
}
...
void STLViewer::on_SavetoolButton_clicked()
{
    ...
    //mXMLwriter = vtkSmartPointer<vtkXMLPolyDataWriter>::New();
    //mXMLwriter->SetFileName(fileName.c_str());
    //mXMLwriter->SetInputData(mSTLReader->GetOutput());
    //mXMLwriter->Write();

    //qDebug() << mXMLwriter->GetFileName();
...
void STLViewer::on_OcclusiontoolButton_clicked()  // Oucclusion 효과를 주기 위한 함수
{
...
{                               // 고러드 쉐이딩에서 하이라이트나 반사광을 표현할 수 없는것을
    ...
    if (mActor != NULL) {
        ...
        mNormalGenerator->SetInputConnection(mSTLReader->GetOutputPort());
...
void STLViewer::setTexture() //png 파일을 입혀주기 위한 함수
{
    ...
    qDebug() << "m_OrigninProp" << mOriginProp;
}