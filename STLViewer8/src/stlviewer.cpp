...
void STLViewer::SetTexture()
{
   ...
   // qDebug() << "m_polyData" << m_PolyData;
    ...
    m_Texture->SetWrap(vtkTexture::VTK_TEXTURE_BLENDING_MODE_ADD);
    m_Texture->GetQuality();
    qDebug() << "Set Wrap" << m_Texture->GetWrap();

    // Transform the texture coordinates
    //vtkSmartPointer<vtkTransformTextureCoords> transformTextureCoords =
    //    vtkSmartPointer<vtkTransformTextureCoords>::New();

    //transformTextureCoords->SetInputConnection(m_STLReader->GetOutputPort());
    //transformTextureCoords->SetScale(1.0, 1.0, 1.0);
    //transformTextureCoords->Update();
    //mMapper->SetInputConnection(transformTextureCoords->GetOutputPort());
    m_Texture->SetBlendingMode(vtkTexture::VTK_TEXTURE_BLENDING_MODE_ADD);

    qDebug() <<"BlendingMode" << m_Texture->GetBlendingMode();
